...
static inline bool IsCmpInst(uint32_t inst)
{
    return (inst >= FIRST_REG_CMP & inst <= LAST_REG_CMP)
...
BCValue BCGen_interpret(BCGen* self, uint32_t fnIdx, BCValue* args, uint32_t n_args, BCHeap* heapPtr)
{
    ...
    for (;;)
    {
        ...
        if (IsCmpInst(lw & INSTMASK))
        {
            if ((state.mode & MODE_CND_SET) && (state.mode & MODE_STICKY_MASK))
            {
