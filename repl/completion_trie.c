...
void CompletionTrie_Init(completion_trie_root_t* self, metac_alloc_t* parentAlloc)
{
    ...
    ARENA_ARRAY_INIT_SZ(completion_trie_node_t, self->Nodes, &self->TrieAllocator, 128)
    self->NodesCount = 4;
    
    ARENA_ARRAY_INIT_SZ(char, self->StringMemory, &self->TrieAllocator, 1024)
                             ...
                             const char* word, uint32_t length)
{
    ...
    while(length)
    {
        ...
        uint32_t newChildCount = PrefNode->ChildCount + 1;
        if (newChildCount & 3 == 0)
        {
            uint32_t oldChildBaseIdx = PrefNode->ChildrenBaseIdx;
            ARENA_ARRAY_ENSURE_SIZE(root->Nodes, root->NodesCount + ALIGN4(newChildCount));
            
            PrefNode->ChildrenBaseIdx = POST_ADD(root->NodesCount, ALIGN4(newChildCount)) / 4;
        }
        
        PrefNode->ChildCount = newChildCount;
