...
#define MetaCNode_TreeWalk_Real(NODE, FN, CTX) \
    ...
    switch(node->Kind)
    {
        ...
        case node_decl_function:
        {
            ...
            if ((metac_node_t)decl_function->YieldType != emptyNode)
                result = MetaCNode_TreeWalk_Real(decl_function->YieldType, walker_fn, ctx);
            if(result)
                 return result;
        ...
        case expr_run:
        {
            ...
            if ((metac_node_t)expr_run->E1 != emptyNode)
                result = MetaCNode_TreeWalk_Real(expr_run->E1, walker_fn, ctx);
             if(result)
                 return result;
        } break;
        ...
        case node_stmt_expr:
        {
            stmt_expr_t* stmt_expr = cast(stmt_expr_t*) node;
            if ((metac_node_t)stmt_expr->Expr != emptyNode)
                result = MetaCNode_TreeWalk_Real(stmt_expr->Expr, walker_fn, ctx);
            if(result)
                 return result;
        } break;
        ...
        case node_stmt_if:
        {
            ...
            if (METAC_NODE(stmt_if->IfCond) != emptyNode)
                result = MetaCNode_TreeWalk_Real(stmt_if->IfCond, walker_fn, ctx);
            if(result)
                return result;
            if (METAC_NODE(stmt_if->IfBody) != emptyNode)
                result = MetaCNode_TreeWalk_Real(stmt_if->IfBody, walker_fn, ctx);
            if(result)
                return result;
            if (METAC_NODE(stmt_if->ElseBody) != emptyNode)
                result = MetaCNode_TreeWalk_Real(stmt_if->ElseBody, walker_fn, ctx);
            if(result)
                return result;
        } break;
